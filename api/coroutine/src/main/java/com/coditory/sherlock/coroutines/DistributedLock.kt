...
interface DistributedLock {
    ...
    suspend fun <T> acquireAndExecute(action: suspend () -> T): AcquireAndExecuteResult<T> {
        return AcquireAndExecuteResult.executeOnAcquired(acquire(), { action() }) { release() }
    }
    ...
    suspend fun <T> acquireAndExecute(
        ...
        return AcquireAndExecuteResult.executeOnAcquired(acquire(duration), { action() }) { release() }
    }

    suspend fun <T> acquireForeverAndExecute(action: suspend () -> T): AcquireAndExecuteResult<T> {
        return AcquireAndExecuteResult.executeOnAcquired(acquireForever(), { action() }) { release() }
    }

    suspend fun <T> releaseAndExecute(action: suspend () -> T): ReleaseAndExecuteResult<T> {
        return ReleaseAndExecuteResult.executeOnReleased(release()) { action() }
    }
